...
for i in range(0, len(df)):
    ...
    if df['thick_EW'][i] < df['thick_NS'][i]:
        ...
        section[i, 1] = - df['dist_EW'][i]
        section[i, 2] = df['dist_EW'][i]
        d_to_pb_M[i, 2] = df['d_to_pb_EW_M'][i]
        d_to_pb_P[i, 2] = df['d_to_pb_EW_P'][i]
    else:
        depth[i, 2] = df['dist_NS'][i]
        section[i, 1] = - df['dist_NS'][i]
        section[i, 2] = df['dist_NS'][i]
...
np.savetxt('map_depth/depth_{}_{}_p1.txt'.format(type_stack, cc_stack), depth, fmt='%10.5f')
np.savetxt('map_depth/section_{}_{}_p1.txt'.format(type_stack, cc_stack), section, fmt='%10.5f')
np.savetxt('map_depth/d_to_pb_{}_{}_M_p1.txt'.format(type_stack, cc_stack), d_to_pb_M, fmt='%10.5f')
np.savetxt('map_depth/d_to_pb_{}_{}_P_p1.txt'.format(type_stack, cc_stack), d_to_pb_P, fmt='%10.5f')
np.savetxt('map_depth/uncertainty_{}_{}_p1.txt'.format(type_stack, cc_stack), uncertainty, fmt='%10.5f')
